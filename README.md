...
| `ext` | `describe` `list` `run` | Console commands this CLI does not wrap — extensions, plugins, your own. `run` warns, records the invocation, and returns the foreign answer inside an envelope (`command_output`, `exit_code`) rather than as its own |
