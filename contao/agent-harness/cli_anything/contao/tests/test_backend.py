...
def make_backend():
    """Create a ContaoBackend instance for testing without triggering _verify_ssh."""
    with patch.object(ContaoBackend, '_verify_ssh'):
        b = ContaoBackend.__new__(ContaoBackend)
        b.host = "example.com"
        b.user = "deploy"
        b.contao_root = "/var/www/contao"
        b.key_path = "/home/user/.ssh/id_ed25519"
        b.port = 22
        b.php_path = "php"
        b._ssh_bin = "/usr/bin/ssh"
        return b


def test_ssh_args_contains_control_master():
    """ControlMaster should be auto and ControlPersist set to 60s."""
    b = make_backend()
    args = b._ssh_args()
    args_str = " ".join(args)
    assert "ControlMaster=auto" in args_str
    assert "ControlPersist=60s" in args_str
